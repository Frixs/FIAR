...
namespace Fiar.Attributes
{
    ...
    public class ValidableModelAttribute : Attribute, IValidableAttribute<object>
    {
        ...
        public ICollection<DataValidationError> Validate(object value, object none)
        {
            ...
            {
                Console.Error.WriteLine("Fatal error during validation");
                Debugger.Break();
            }
            ...
            foreach (PropertyInfo pi in properties)
            {
                ...
                foreach (var attribute in pi.GetCustomAttributes(typeof(BaseValidateAttribute)))
                {
                    ...
                    {
                        Console.Error.WriteLine($"Fatal error during validation - Some validation attributes are not specified in this switch (Type: {attribute.GetType()}).");
                        Debugger.Break();
                    }
                }
            }
